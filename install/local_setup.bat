...
:_colcon_run_ordered_commands
  ...
  if "%COLCON_PYTHON_EXECUTABLE%" NEQ "" (
    ...
    set "_colcon_python_executable=C:\Users\edvar\programming\ros2_ws\.pixi\envs\default\python.exe"
  ...
  set "_colcon_python_executable=%_colcon_python_executable:)=^)%"
  for /f "delims=" %%c in ('""%_colcon_python_executable%" "%~1_local_setup_util_bat.py" bat"') do (
    if "%COLCON_TRACE%" NEQ "" (
      echo %%c
    )
